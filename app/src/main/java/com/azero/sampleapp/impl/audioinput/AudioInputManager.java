...
import com.azero.sampleapp.manager.DispatcherUIEvent;
...
public class AudioInputManager extends InputManager {
    ...
    public AudioInputManager(Context context,Record record) {
        ...
        openDenoiseManager = new OpenDenoiseManager(context, record, true, new OpenDenoiseManager.DenoiseCallback() {
            @Override
            ...
            public void onWakeUp(String wakeWord) {
                DispatcherUIEvent.getInstance().dispatchWakeUpEvent();
    ...
    @Override
    public void startvoip(@NonNull VoipInputConsumer voipInputConsumer) {
        onStartVoIp(voipInputConsumer);
    }

    @Override
    public void stopvoip(@NonNull VoipInputConsumer voipInputConsumer) {
    ...
    public void onStopVoIp(@NonNull VoipInputConsumer consumer) {
    ...
    public void onStartVoIp(@NonNull VoipInputConsumer consumer) {
    ...
    private void notifyDataAvailableToVoIpInputConsumers(byte[] data, int size) {
        synchronized (mVoipInputConsumers) {
            for (VoipInputConsumer consumer : mVoipInputConsumers) {
